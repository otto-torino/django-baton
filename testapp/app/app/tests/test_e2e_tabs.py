...
class TestBatonTabs(TestCase):
    ...
    def test_tabs(self):
        ...
        # tabs number
        tabs_li = self.driver.find_elements(By.CSS_SELECTOR, ".nav-tabs .nav-item")
        ...
        description_att = self.driver.find_element(
            By.CSS_SELECTOR, ".tab-fs-attachments .description"
        )
        ...
        self.assertEqual(
            description_att.get_attribute("innerText"),
            "Add as many attachments as you want",
        )

        # tabs groups && inlines
        attachments_summary = self.driver.find_element(
            By.ID, "id_attachments_summary_en"
        )
        self.assertEqual(attachments_summary.is_displayed(), True)
        inlines = self.driver.find_element(
            By.CSS_SELECTOR,
            "#group-fs-attachments--inline-attachments .inline-related .module",
        )
        inline_title = inlines.find_element(By.TAG_NAME, "h2")
        attachments_rows = inlines.find_elements(
            By.CSS_SELECTOR, ".dynamic-attachments"
        )
    ...
    def test_detect_tab_error(self):
        ...
        button = self.driver.find_element(
            By.CSS_SELECTOR, "input[type=submit][name=_continue]"
        )
        time.sleep(1)
