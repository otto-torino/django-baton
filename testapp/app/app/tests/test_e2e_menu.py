...
class TestBatonMenu(TestCase):
    ...
    def test_menu(self):
        ...
        self.assertEqual(root_voices[1].find_element(By.CLASS_NAME, 'fa-lock').is_displayed(), True)
        self.assertEqual(root_voices[1].find_element(By.CLASS_NAME, 'has-children').get_attribute('innerText'), 'Authentication')
        ...
        self.assertEqual(auth_children[0].find_element(By.TAG_NAME, 'a').get_attribute('href'), 'http://localhost:8000/admin/auth/user/')
        self.assertEqual(auth_children[1].is_displayed(), False)
        self.assertEqual(auth_children[1].find_element(By.TAG_NAME, 'a').get_attribute('href'), 'http://localhost:8000/admin/auth/group/')
        # open submenu on click
        root_voices[1].click()
        self.assertEqual(auth_children[0].is_displayed(), True)
        self.assertEqual(auth_children[0].find_element(By.TAG_NAME, 'a').get_attribute('innerText'), 'Users')
        self.assertEqual(auth_children[1].is_displayed(), True)
        self.assertEqual(auth_children[1].find_element(By.TAG_NAME, 'a').get_attribute('innerText'), 'Groups')

        # news menu title volice
        self.assertEqual(root_voices[2].find_element(By.CSS_SELECTOR, 'span.has-children').get_attribute('innerText'), 'NEWS')
        ...
        self.assertEqual(news_children[0].find_element(By.TAG_NAME, 'a').get_attribute('href'), 'http://localhost:8000/admin/news/category/')
        self.assertEqual(news_children[1].is_displayed(), True)
        self.assertEqual(news_children[1].find_element(By.TAG_NAME, 'a').get_attribute('href'), 'http://localhost:8000/admin/news/news/')
        ...
        # tools voice
        self.assertEqual(root_voices[3].find_element(By.CSS_SELECTOR, 'span.has-children').get_attribute('innerText'), 'TOOLS')
