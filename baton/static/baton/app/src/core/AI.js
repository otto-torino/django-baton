...
const AI = {
    ...
    addSummarization(fieldName, conf) {
        ...
        const summarizeButton = $('<a />', { class: 'btn btn-sm btn-secondary me-2', href: '#' })
            .on('click', function () {
                self.handleSummarization(field, targetLabel, conf)
            })
            .prepend($('<i />', { class: 'fa fa-align-justify' }))
...
<input type="checkbox" name="useBulletedList" id="${field.attr('id')}_useBulletedList" value="1" ${conf?.useBulletedList ? 'checked' : ''} class="form-check-input" />
</div>
`
